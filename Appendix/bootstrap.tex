...
Several of the settings for the {\bf de\_mapper.m} functions are automatically generated based on other parameters and others are set at the start of the function script.
The resolution of the resulting map is entirely determined by the resolution of the current LWPC lookup tables. 
The other settings have been chosen based on either the normal WWLLN operation standards or based on what is found to give the best results.

The distance variables control the range of stroke-station pairs to use in calculating the relative detection efficiency.
They are currently set to a minimum of 500~km and a maximum of 20~Mm.
...
exclude $<500$ km and $> 20000$ km

station on at 500 counts

station\_min to 5th lowest station
